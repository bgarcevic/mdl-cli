// Copyright (c) 2026 SQLBI Corp. Portions of this file are used under the MIT License.
// The full license notice is reproduced in THIRD-PARTY-NOTICES.md in the repository root.

namespace Tomix.Core.Dax.Engine;
...
internal static class DaxLexer
{
    ...
    public static List<DaxToken> Tokenize(string source)
    {
        ...
        while (index < source.Length)
        {
            ...
            {
                index = ScanBracketed(source, index);
    ...
    /// <summary>
    /// Scans a bracketed name, treating a doubled closing bracket as an escaped bracket, so
    /// <c>[Weird ]] Name]</c> is one token. An unterminated name consumes to end of input.
    /// </summary>
    private static int ScanBracketed(string source, int index)
    {
